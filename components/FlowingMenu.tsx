    ...
    return (
        <div className="w-full h-full overflow-hidden">
            <nav className="flex flex-col h-full m-0 p-0">
                {items.map((item, idx) => (
                    <MenuItemComponent key={idx} {...item} onItemClick={onItemClick} />
                ))}
            </nav>
        </div>
    );
...
function MenuItemComponent({ link, text, image, onItemClick }: MenuItemComponentProps) {
    ...
    const repeatedMarqueeContent = image ? Array.from({ length: 4 }).map((_, idx) => (
        ...
        <span key={idx} className="text-[#060010] uppercase font-normal text-[4vh] leading-[1.2] p-[1vh_2vw_0] whitespace-nowrap">
            {text}
        </span>
    ));

    return (
        <div className="flex-1 relative overflow-hidden text-center shadow-[0_-1px_0_0_rgba(255,255,255,0.1)]" ref={itemRef}>
            <a
