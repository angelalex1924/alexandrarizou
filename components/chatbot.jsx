...
const AIChatbot = () => {
  ...
  const isGreekPath = !pathname.startsWith('/en') // Greek if not English path
                ...
                for (let i = 0; i < models.length; i++) {
                  try {
                    ...
                    response = await fetch(currentConfig.baseUrl, {
                      ...
                      // Timeout για γρηγορότερη απόκριση
                      timeout: 30000, // 30 δευτερόλεπτα
                      body: JSON.stringify({
  ...
  return (
    <div className="fixed bottom-6 right-6 z-30 chatbot-container" data-open={isOpen} style={{ display: isMobile ? (isOpen ? 'block' : 'none') : 'block' }}>
      <AnimatePresence>
        {isOpen ? (
          <motion.div
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-lg border border-white/30 dark:border-gray-700/50 focus-within:border-blue-500/70 focus-within:ring-2 focus-within:ring-blue-500/30 transition-all bg-white/80 dark:bg-gray-800/80 backdrop-blur-md">
                  <Input
                    ...
                    onFocus={() => {
                      ...
                      if (isMobile) {
                        setTimeout(() => scrollToBottomAggressive(), 100)
                        setTimeout(() => scrollToBottomAggressive(), 300)
                      }
                    }}
          ...
          >
            ...
            <div className="relative">
              {/* Matte golden bubble background, no glow */}
              <div className="relative w-14 h-14 rounded-full overflow-hidden flex items-center justify-center bg-gradient-to-br from-[#d4af37] to-[#f0c24c]">
                {/* Icon - custom SVG, centered */}
                <span className="relative z-10 flex items-center justify-center w-full h-full">
                  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-7 h-7">
                    <path d="M8 10.5H16" stroke="#1a1a1a" strokeWidth="1.5" strokeLinecap="round"></path>
                    <path d="M8 14H13.5" stroke="#1a1a1a" strokeWidth="1.5" strokeLinecap="round"></path>
                    <path d="M17 3.33782C15.5291 2.48697 13.8214 2 12 2C6.47715 2 2 6.47715 2 12C2 13.5997 2.37562 15.1116 3.04346 16.4525C3.22094 16.8088 3.28001 17.2161 3.17712 17.6006L2.58151 19.8267C2.32295 20.793 3.20701 21.677 4.17335 21.4185L6.39939 20.8229C6.78393 20.72 7.19121 20.7791 7.54753 20.9565C8.88837 21.6244 10.4003 22 12 22C17.5228 22 22 17.5228 22 12C22 10.1786 21.513 8.47087 20.6622 7" stroke="#1a1a1a" strokeWidth="1.5" strokeLinecap="round"></path>
                  </svg>
                </span>
              </div>
              ...
              <div className="absolute -bottom-0.5 -right-0.5">
                <div className="relative w-4 h-4">
                  {/* Matte golden badge, no glow */}
                  <div className="absolute inset-0 rounded-full bg-[#d4af37]"></div>
                </div>
              </div>
            </div>
            ...
            {isHovered && (
              ...
              >
                <div className="bg-gradient-to-r from-[#d4af37] to-[#f0c24c] text-gray-900 text-sm py-2 px-4 rounded-lg shadow-lg whitespace-nowrap border border-[#d4af37]/20 backdrop-blur-sm font-semibold">
                  Chat με το Alexandra Rizou AI
                  <div className="absolute top-1/2 right-0 transform translate-x-1/2 -translate-y-1/2 rotate-45 w-2.5 h-2.5 bg-[#f0c24c]"></div>
                </div>
              </motion.div>
            )}
          </motion.div>
