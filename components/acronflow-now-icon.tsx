...
interface AcronFlowNowIconProps {
  size?: number;
  className?: string;
}

export const AcronFlowNowIcon: React.FC<AcronFlowNowIconProps> = ({ 
  size = 20, 
  className = "" 
}) => {
  return (
    ...
    >
      <g style={{ opacity: 1 }} id="logogram" transform="translate(0, 0) rotate(0)">
        <path d="M0 20C0 12.5231 0 8.78461 1.60769 6C2.66091 4.17577 4.17577 2.66091 6 1.60769C8.78461 0 12.5231 0 20 0C27.4769 0 31.2154 0 34 1.60769C35.8242 2.66091 37.3391 4.17577 38.3923 6C40 8.78461 40 12.5231 40 20C40 27.4769 40 31.2154 38.3923 34C37.3391 35.8242 35.8242 37.3391 34 38.3923C31.2154 40 27.4769 40 20 40C12.5231 40 8.78461 40 6 38.3923C4.17577 37.3391 2.66091 35.8242 1.60769 34C0 31.2154 0 27.4769 0 20Z" fill="#63DB00"/>
...
export function AcronFlowNowLogo({ 
  iconSize = 24, 
  textSize = "text-lg",
  className = "",
  showBadge = false,
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      <div className="relative">
        <AcronFlowNowIcon size={iconSize} />
        {showBadge && (
          <div className="absolute -top-1 -right-1 w-4 h-4 bg-gradient-to-r from-[#63DB00] to-[#4CAF50] rounded-full flex items-center justify-center shadow-lg border border-white">
            <span className="text-white text-xs font-bold">{badgeCount}</span>
          </div>
        )}
      </div>
      
      {/* Text Logo with AcronFlow CRM Style */}
      <div className="flex flex-col items-start gap-0">
        <span 
          className={`font-milker font-medium tracking-wide text-gray-800 dark:text-white ${textSize}`}
          style={{
        ...
        {/* NOW Badge with AcronFlow CRM Style */}
        <div className="relative flex items-center gap-0.5 -mt-0.5">
          {/* Accent dots with AcronFlow Now colors */}
          <div className="w-1 h-1 rounded-full bg-gradient-to-br from-[#63DB00] to-[#4CAF50] shadow-md shadow-[#63DB00]/30 animate-pulse"></div>
          <div className="w-1 h-1 rounded-full bg-gradient-to-br from-[#4CAF50] to-[#63DB00] shadow-md shadow-[#4CAF50]/30 animate-pulse delay-300"></div>
          <div className="w-1 h-1 rounded-full bg-gradient-to-br from-[#7DE000] to-[#63DB00] shadow-md shadow-[#7DE000]/30 animate-pulse delay-600"></div>
          
          {/* NOW text with AcronFlow Now colors */}
          <span 
            className="ml-0.5 text-[8px] font-milker font-medium uppercase tracking-wide text-[#63DB00] dark:text-[#4CAF50]"
            style={{
              fontFamily: "'Milker', 'Arial', sans-serif"
            }}
          >
