...
export function ThemeLampToggle({ className }: ThemeLampToggleProps) {
  ...
  return (
    <button
      ...
      onClick={toggleTheme}
      className={cn(
        "relative rounded-full transition-all duration-300 active:scale-95 flex items-center justify-center",
        className
      )}
      style={{ 
        position: 'relative',
    ...
    >
      <div className="relative z-10 flex items-center justify-center w-full h-full" style={{ overflow: 'visible' }}>
        <svg
        ...
        />
        {/* Sun rays - Animated, visible in light mode */}
        <motion.g
          animate={{
            scale: isDark ? 0.5 : 1,
            opacity: isDark ? 0 : 1,
